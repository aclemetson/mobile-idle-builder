    ...
    public partial struct TutorialSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            if (tutorial.CurrentStepIndex != _diagLastStep)
            {
                ...
                _diagLastCount = -1;
                GameLogger.Debug($"[TutorialSystem] Now evaluating step {tutorial.CurrentStepIndex}: '{step.id}'  condition={step.advanceCondition?.type}");
            }
                ...
                && step.advanceCondition.items != null && step.advanceCondition.items.Count > 0)
            {
                ...
                {
                    _diagLastCount = have;
                    GameLogger.Debug($"[TutorialSystem] Step '{step.id}' — item {req.itemId}: {have}/{req.quantity}  IsActive={tutorial.IsActive}  FlowSteps={flow.steps.Length}");
                }
            }
            ...
            string nextId = tutorial.IsActive && tutorial.CurrentStepIndex < flow.steps.Length
                ...
                : "(complete)";
            GameLogger.Info($"[TutorialSystem] Advanced to step {tutorial.CurrentStepIndex}: {nextId}");
        }
