...
- **Model.** A consumer is *connected* if its footprint is within any generator's `InfluenceRadius` (Euclidean edge-gap). Global `Supply` = Σ generator `MaxEV`; global `Draw` = Σ connected consumers' `DrawEV × AppliedPowerMult`. `Ratio = Draw <= Supply ? 1 : Supply/Draw`. Each consumer's `ThrottleRatio` = connected ? `Ratio` : 0.
