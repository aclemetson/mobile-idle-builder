...
**Status:** Phase 1 PR #57 (merged-or-open, branch `feature/multi-grids-p1`) — data + save model. Phase 2 done (branch `feature/multi-grids-p1`, this PR) — `SiteService` switch/unlock + per-site idle aggregation + dev console. Phases 3-4 pending. **Scene wiring required:** add a `SiteService` component to the GameScene services GameObject (alongside `ResearchService`/`GridSaveService`) before the in-editor playtest.
