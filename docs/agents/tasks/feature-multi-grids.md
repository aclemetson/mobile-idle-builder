...
**Status:** Phase 1 PR #57 (open, `feature/multi-grids-p1`) — data + save model. Phase 2 PR #58 (open, `feature/multi-grids-p2`, stacked on #57) — `SiteService` switch/unlock + per-site idle aggregation + dev console. Phase 3 (open, `feature/multi-grids-p3`, stacked on #58) — `sites-panel` UI + `SitesSubController` (Travel/Unlock/two-tap confirm). Phase 4 pending. **Scene wiring required (manual, Unity Editor):** (a) add a `SiteService` component to the GameScene services GameObject (alongside `ResearchService`/`GridSaveService`); (b) add a `SitesSubController` component to the HUD GameObject (alongside `HUDController`/`PrestigeShopSubController`). Neither functions until wired.
