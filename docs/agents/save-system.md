...
| `siteSnapshots` (List<IdleCollectionSnapshot>) | per-site offline snapshots, index = site index. Mirrors `idleSnapshot` at the active index; inactive sites keep producing from their stored entries. Populated by `GridSaveService.MirrorActiveSiteSnapshot` on every flush/load. Run state (cleared on prestige in Phase 4) |
...
- Inactive sites are NEVER live ECS (load-bearing constraint). Dev console: `site list` / `site switch <n>` / `site unlock <id>`.
