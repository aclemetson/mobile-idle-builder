...
- Inactive sites are NEVER live ECS (load-bearing constraint). Dev console: `site list` / `site switch <n>` / `site unlock <id>`. UI: `sites-panel` + `SitesSubController` (Travel/Unlock).
